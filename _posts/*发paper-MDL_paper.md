...
Figure 1. 处理高光谱图像分类的多尺度和多级光谱空间特征融合网络（MSSN）的流程图， 

图1为所提出的方法的流程图。 这里使用的多任务网络取得了9×9×通道的输入。 网络的第一部分是具有两个残差单位和全局平均池层的编码器/特征提取器。 在提取潜在特征后，具有软MAX功能的完全连接的层用作分类器并将概率输出到已知类。 这里的重建任务使用碎屑层逐渐增加潜在特征的空间尺寸。 重建任务的输出是9×9×信道实例，这应该通过最小化1滚动来类似于输入数据。 EVT模拟重建损失以分离已知和未知的类别 
